...
async function loadCategory(page, param) {
  ...
  var url =
    "http://localhost:8080/api/category/public/search?page=" +
    page +
    "&size=" +
    size +
    "&q=" +
  ...
  for (i = 0; i < list.length; i++) {
    main += `<tr>
      ...
      list[i].isPrimary == true
        ? '<span class="dmchinh"> <i class="fa fa-check-circle"></i> Danh mục chính</span>'
        : ""
    }</td>
                    <td>${
                      list[i].categoryParentName == null
                        ? ""
                        : list[i].categoryParentName
                    }</td>
                    <td class="sticky-col">
                        <i onclick="deleteCategory(${
                          list[i].id
                        })" class="fa fa-trash-alt iconaction"></i>
                        <a href="addcategory?id=${
                          list[i].id
                        }"><i class="fa fa-edit iconaction"></i></a>
                    </td>
                </tr>`;
  }
  ...
  for (i = 1; i <= totalPage; i++) {
    mainpage += `<li onclick="loadCategory(${
      Number(i) - 1
    },${param})" class="page-item"><a class="page-link" href="#listsp">${i}</a></li>`;
  }
...
async function loadACategory() {
  ...
  if (id != null) {
    ...
    result.isPrimary == true
      ? (document.getElementById("primaryCate").checked = true)
      : false;
    ...
    if (result.categoryParentId != null) {
      ...
      document.getElementById("select2-listdpar-container").innerText =
        result.categoryParentName;
    }
  }
}
