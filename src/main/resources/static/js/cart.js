...
async function loadAllCart() {
  ...
  for (i = 0; i < list.length; i++) {
    main += `<tr>
                    <td>
                        <a href="detail?id=${list[i].product.id}&name=${
      list[i].product.alias
    }"><img class="imgprocart" src="${list[i].product.imageBanner}"></a>
                        <div class="divnamecart">
                            <a href="detail?id=${list[i].product.id}&name=${
      list[i].product.alias
    }" class="nameprocart">${list[i].product.name}</a>
                            <p class="sizecart">${list[i].color.colorName} / ${
      list[i].size.sizeName
    }</p>
                        </div>
                    </td>
                    <td><p class="boldcart">${formatmoney(
                    ...
                    <td>
                        <div class="tdpricecart">
                            <p class="boldcart">${formatmoney(
                              list[i].quantiy * list[i].product.price
                            )}</p>
                            <p onclick="remove(${
                              list[i].color.id
                            })" class="delcart"><i class="fa fa-trash-o facartde"></i></p>
                        </div>
                    </td>
                </tr>`;
  ...
  for (i = 0; i < list.length; i++) {
    main += `<tr>
        <td>
            <a href="detail?id=${list[i].product.id}&name=${
      list[i].product.alias
    }"><img class="imgprocmobile" src="${list[i].product.imageBanner}"></a>
        </td>
        <td class="tdinforcart">
            <a href="detail?id=${list[i].product.id}&name=${
      list[i].product.alias
    }" class="nameprocmobile">${list[i].product.name}</a>
            <p class="sizecmobile">${list[i].color.colorName} / ${
      list[i].size.sizeName
    }</p>
        ...
        <td class="tdinforcart">
            <i onclick="remove(${
              list[i].color.id
            })" class="fa fa-trash-o facartde cartdels"></i>
            <p class="boldcart pricecmobile">${formatmoney(
              list[i].quantiy * list[i].product.price
            )}&ThinSpace;</p>
        </td>
    </tr>`;
