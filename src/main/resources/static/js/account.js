...
async function confirmAccount() {
  ...
  var url =
    "http://localhost:8080/api/active-account?email=" + email + "&key=" + key;
...
// Unified load handler: only call account-specific loaders on account page
window.addEventListener("load", function () {
  loadMenu();
  var path = window.location.pathname;
  // Only run these heavy loaders if we are on the account page
...
// Removed duplicated load event listener above which caused multiple menu/category injections
