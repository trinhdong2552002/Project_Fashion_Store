...
async function loadProductComment() {
  ...
  if (id != null) {
    var url =
      "http://localhost:8080/api/product-comment/public/find-by-product?idproduct=" +
      id;
    ...
    for (i = 0; i < list.length; i++) {
      ...
      main += `<div class="singlectlct">
            <div class="row">
                <div class="col-11">
                    <div class="d-flex nguoidangctl justify-content-start">
                        <img class="avtuserdangctl" style="width: 10%; height:100%;" src="image/avatar.webp">
                        <div class="d-flex flex-column ms-2" style="width: 100%;">
                             <span class="usernamedangctl m-0">
                                ${
                                  list[i].user.fullname == null
                                    ? "Người dùng"
                                    : list[i].user.fullname
                                }
                            </span>
                            <span class="ngaytraloi m-0">${
                              list[i].createdDate
                            }</span>
                            <span class="starcmts m-0">${star}</span>
                        </div>
                    </div>
                        
                    <div class="contentctlct" style="font-size:18px; margin-top:30px;">
                        ${list[i].content}
                        ${
                          list[i].isMyComment == true
                            ? `<span class="starcmts"><i onclick="deleteComment(${list[i].id})" class="fa fa-trash pointer text-danger fs-5"></i></span>`
                            : ""
                        }
                    </div>
...
async function saveComment() {
  ...
  var listLinkImg = await uploadMultipleFile(
    document.getElementById("choosefilecmt")
  );
  var comment = {
    star: star,
    content: noidungbl,
